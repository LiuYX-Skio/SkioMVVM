...
//    RefreshLayout setEnableNestedScrollOnly(boolean enabled);
    ...
    /**
     * Display load more animation and trigger load more event, Delayed start.
     * @param isUpRefresh 是否禁用下拉刷新后滑动底部是否可以上拉
     */
    void setUpRefresh(boolean isUpRefresh);
    /**
     * Display load more animation and trigger load more event, Delayed start.
     * @param isDownRefresh 是否禁用上拉加载后滑动顶部是否可以下拉
     */
    void setDownLode(boolean isDownRefresh);
