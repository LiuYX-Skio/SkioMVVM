...
//    protected boolean mManualNestedScrolling = false;//是否手动设置过 NestedScrolling，用于智能开启
    ...
    private boolean isUpRefresh=false;//是否可以下拉刷新
    private boolean isDownLode=false;//是否可以上拉加载
    private boolean isStopRefresh=false;//是否禁用下拉刷新后滑动底部是否可以上拉
    private boolean isStopLode=false;//是否禁用上拉加载后滑动顶部是否可以下拉
...
//            }
            ...
            if (mLoadMoreListener != null) {
                ...
                finishLoadMore(300);//如果没有任何加载监听器，两秒之后自动关闭
            }
            if (mRefreshFooter != null) {
...
//            reboundAnimator.setDuration(20000);
    ...
    protected void moveSpinnerInfinitely(float spinner) {
        Log.w("数据接收","hhhh"+isDownLode+"---"+isUpRefresh);

        if (spinner < 0 && (mState == RefreshState.Loading
                || (mEnableFooterFollowWhenNoMoreData && mFooterNoMoreData && isEnableRefreshOrLoadMore(mEnableLoadMore))
                || (mEnableAutoLoadMore && !mFooterNoMoreData && isEnableRefreshOrLoadMore(mEnableLoadMore)))) {
            if(isStopRefresh==false&&isUpRefresh){
                return;
            }

        }else if (spinner >= 0) {
            if(isStopLode==false&&isDownLode){
                return;
            }
        }
        Log.w("数据接收","jjjjjj"+isDownLode+"---"+isUpRefresh);
                final View thisView = this;
            ...
            postDelayed(new Runnable() {
                ...
                public void run() {
                    if (mLoadMoreListener != null) {
                        ...
                        finishLoadMore(1000);//如果没有任何加载监听器，1秒之后自动关闭
                    }
                    final OnLoadMoreListener listener = mOnMultiPurposeListener;
                    if (listener != null) {
...
//    }
     ...
     */
    @Override
    public SmartRefreshLayout finishLoadMore(final int delayed, final boolean success, final boolean noMoreData) {
        postDelayed(new Runnable() {
            @Override
            public void run() {
    ...
    public void setUpRefresh(boolean isUpRefresh) {
        this.isStopRefresh=isUpRefresh;
    }

    @Override
    public void setDownLode(boolean isDownRefresh) {
        this.isStopLode=isDownRefresh;
    }
